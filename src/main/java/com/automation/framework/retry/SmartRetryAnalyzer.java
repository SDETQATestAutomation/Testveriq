...
/**
 * Smart Retry Analyzer - Implements TestNG's IRetryAnalyzer interface
 * Part of the Hybrid Retry System that supports both annotation-based and configuration-based retry logic
 * Provides intelligent failure analysis and retry decision making
 * 
...
public class SmartRetryAnalyzer implements IRetryAnalyzer {
    ...
    private boolean handleConfigurationBasedRetry(ITestResult result, String testKey) {
        AtomicInteger retryCount = retryCountMap.computeIfAbsent(testKey, k -> new AtomicInteger(0));
        int currentRetryCount = retryCount.get();
        int maxRetries = config.getPropertyAsInt("retry.max.attempts", FrameworkConstants.DEFAULT_RETRY_COUNT);
    ...
    private void captureRetryScreenshot(String testKey, int retryAttempt) {
        try {
            String screenshotPath = com.automation.framework.utils.ScreenshotUtil.captureRetryScreenshot(testKey, retryAttempt);
